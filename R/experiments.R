...
#' @param signal_strength Signal multiplier for \code{\link{generate_data}}
#' @param noise_sd Noise level for \code{\link{generate_data}}
    ...
    for (noise in noise_levels) {
      for (theta in c(0.2, 0.5, 0.8)) {
        rf_cors <- replicate(n_replicates, {
          d <- generate_data(500, p, signal, noise)
          rf <- ranger::ranger(y ~ ., data = cbind(y = d$y[1:400], d$X[1:400, ]),
